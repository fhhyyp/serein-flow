...
<template>
  <main class="operations-console" aria-label="SereinFlow operations console">
    ...
    <div v-if="snapshotRun" class="snapshot-dialog-backdrop" role="presentation" @click.self="closeSnapshot"><section class="snapshot-dialog" role="dialog" aria-modal="true" :aria-label="t('console.snapshotTitle')"><header><div class="snapshot-dialog__heading"><p class="operations-console__eyebrow">{{ shortId(snapshotRun.id) }}</p><h2>{{ t('console.snapshotTitle') }}</h2></div><button class="icon-button" type="button" :title="t('command.close')" :aria-label="t('command.close')" @click="closeSnapshot"><X :size="16" /></button></header><div v-if="isSnapshotLoading" class="snapshot-dialog__status">{{ t('console.snapshotLoading') }}</div><div v-else-if="snapshotError" class="snapshot-dialog__status snapshot-dialog__status--error">{{ snapshotError }}</div><RunSnapshotViewer v-else :run-id="snapshotRun.id" :definition="snapshot" :outputs="snapshotOutputs" :outputs-error="snapshotOutputsError" :events="snapshotEvents" :events-error="snapshotEventsError" :is-debug-run="snapshotRun.executionKind === 'debug'" /></section></div>
