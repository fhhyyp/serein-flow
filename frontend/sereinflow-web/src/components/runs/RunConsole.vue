...
const selectedInterfaceProject = computed(() => workspaces.value.find((item) => item.project.id === interfaceForm.projectId))
...
function ensureInterfaceFormTarget(): void {
  if (!workspaces.value.some((workspace) => workspace.project.id === interfaceForm.projectId)) {
    interfaceForm.projectId = workspaces.value.find((workspace) => workspace.flows.length > 0)?.project.id ?? ''
  }
...
function resetInterfaceForm(): void {
  ...
  interfaceForm.projectId = workspaces.value.find((workspace) => workspace.flows.length > 0)?.project.id ?? ''
  interfaceForm.flowId = workspaces.value.find((workspace) => workspace.project.id === interfaceForm.projectId)?.flows[0]?.id ?? ''
}
  ...
  <main class="operations-console" aria-label="SereinFlow operations console">
    ...
    <section class="operations-console__content">
      ...
      <template v-else-if="activeView === 'projects'">
        ...
        <section class="project-catalog"><article v-for="workspace in workspaces" :key="workspace.project.id" class="project-catalog__item"><div><h2>{{ workspace.project.name }}</h2><p>{{ t('console.projectCreatedAt', { date: formatDate(workspace.project.createdAt) }) }}</p></div><dl><div><dt>{{ t('project.flows') }}</dt><dd>{{ workspace.flows.length }}</dd></div><div><dt>{{ t('console.canvasCount') }}</dt><dd>{{ workspace.flows.reduce((total, flow) => total + (flow.canvasCount ?? 0), 0) }}</dd></div><div><dt>{{ t('console.nodeCount') }}</dt><dd>{{ workspace.flows.reduce((total, flow) => total + (flow.nodeCount ?? 0), 0) }}</dd></div></dl><button class="command-button quiet" type="button" :disabled="workspace.flows.length === 0" @click="openFlow(workspace, workspace.flows[0]?.id)"><FilePenLine :size="15" /><span>{{ t('console.openEditor') }}</span></button></article><p v-if="workspaces.length === 0" class="operations-console__empty">{{ t('runs.emptyProjects') }}</p></section>
