...
API 只接受 JSON payload，并校验运行状态、topic、`messageId`/`Idempotency-Key`、大小、入口注册状态、`ExternalIngress` 和 `contractId`。返回 `202 Accepted` 表示消息已进入 Worker 本地 Broker，不表示下游流程已经执行完成；流程执行结果继续通过运行事件和最终结果观察。
